...
/* -- TODO */
%token TUNIT TWIRE UNITV EMP UNION TNAT TBOOL
%token TRUE FALSE
%token SUBEQ AND TOP
%token PAR SEC
%token SINGL PS FIX LAM MATCH WITH BEGIN END MEND LET 
%token IN AT WIRE PLUSPLUS PLUS GT WAPP WAPS WFOLD BIGARR WCOPY FROM AS
...
| SYSOP var values                                  { astnd( Ast.E_sysop($2, $3)) 1 3 
}
