...
{
    long int load;
    long int input_setup;
    long int compute_gates;
    long int output_setup;

    //long int totalconfig;
    long int total;
};
...
public:
	BOOL Init();
	BOOL EstablishConnection(vector<string> &addrs, vector<USHORT> &ports,
				 int num_parties, int self_id, int num_and_gates);
	void Cleanup();
	/*void DumpOTStuff();
	  void LoadOTStuff();*/
	int Run(struct perf_data& d);
	const vector<int>& GetOutput(){ return m_vOutputs; }

	void GetSharesOutput(vector<char>& shares);
